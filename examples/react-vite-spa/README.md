# React (Vite SPA) Integration Example

Verified against Vite 8.2, React 19.2, `ai-visibility` 0.3.1.
...
3. Optionally, render *additional* schema client-side as a plain
   `<script>` element for things that are genuinely dynamic per route — but
   that only reaches crawlers that run your JS. `App.tsx` demonstrates this
   with the caveat stated inline.
...
[React Router example](../react-router-app) or the
[Nuxt example](../nuxt-app) for what that looks like, or deploy this SPA
behind a thin Express/Workers layer that at least fronts the static files.

## Files

```
vite.config.ts   # build-time robots.txt/llms.txt + index.html JSON-LD injection
src/App.tsx      # client-side JSON-LD via a plain <script>, with the caveat above
```
